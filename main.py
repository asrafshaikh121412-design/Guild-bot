...
async def on_message(message):
    ...
    allowed_ranks = ["OFFICER", "AGL", "LEADER"]
    ...
    if message.content.startswith("?"):
        ...
        # ❌ Permission check
        if not is_admin:
            if not user_data or user_data.get("rank", "").upper() not in allowed_ranks:
                await message.channel.send("❌ Only Officer / AGL / Leader can use this")
                return
        ...
        command = parts[0][1:].strip()  # remove "?"

        # =========================
        # 🔥 REMOVE ROLE SYSTEM
        # =========================
        if command.lower().startswith("remove"):
            role_name = command[6:].strip().upper()  # removeFE01 → FE01

            role = discord.utils.get(message.guild.roles, name=role_name)

            if role:
                await member.remove_roles(role)
                await message.channel.send(f"🗑 {member.mention} removed from {role.name}")
            else:
...
async def on_member_join(member):
    try:
        if not member.display_name.startswith("FE |"):
            await member.edit(nick=f"FE | {member.name}")
