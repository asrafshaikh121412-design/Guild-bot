...
async def guild_list(interaction: discord.Interaction):
    ...
    for g in guilds:
        ...
        total = len(members)
        officer = len([m for m in members if m["rank"] == "Officer"])
        agl = len([m for m in members if m["rank"] == "AGL"])
        leader = len([m for m in members if m["rank"] == "Leader"])
...
#SET RANK
@bot.tree.command(name="set_rank", description="Set guild rank")
async def set_rank(interaction: discord.Interaction, user: discord.Member, rank: str):
    ...
    success = database.edit_user(str(user.id), rank=rank)

    if not success:
        await interaction.followup.send("❌ User not found")
        return

    await interaction.followup.send(f"✅ {user.mention} is now `{rank}`")
