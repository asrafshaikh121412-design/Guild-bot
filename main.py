...
# 📊 GUILD LIST
@bot.tree.command(name="guild_list", description="Guild list")
async def guild_list(interaction: discord.Interaction):
    ...
    for g in guilds:
        ...
        embed.add_field(name=g, value=f"Members: {members}", inline=False)

    await interaction.followup.send(embed=embed)

# 👥 GUILD MEMBERS
@bot.tree.command(name="guild_members", description="Guild members")
async def guild_members(interaction: discord.Interaction, guild: str):
    ...
    await interaction.followup.send(f"👥 {guild.upper()}:\n\n{msg}")
