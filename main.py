...
#search 
@bot.tree.command(name="search", description="Search player by UID")
async def search(interaction: discord.Interaction, uid: str):
    ...
    try:
        ...
        if "nickname" in data:
            ...
            # 🔥 Auto save in DB
            database.bind_user(
                str(interaction.user.id),  # temp user id
                name,
                uid,
                "Unknown Guild",
                "Unknown Rank",
    ...
    if data:
        ...
        embed.add_field(name="UID", value=uid, inline=False)
        embed.add_field(name="Guild", value=f"{guild} ({rank})", inline=True)
        embed.add_field(name="Source", value="Database 💾", inline=True)

        await interaction.followup.send(embed=embed)
    else:
        await interaction.followup.send("❌ Player not found (API + DB)")
# 📊 Guild List
